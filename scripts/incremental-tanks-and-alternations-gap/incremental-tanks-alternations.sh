...
usage() {
  cat << EOF
Usage: $0 [semantics] [max_tanks] [max_alternations] [gap] [time]
...
csv_file="${0::-3}-$semantics-gap=$gap-time=$time-($current_date).csv"
{
  ...
  echo -n "DENOT_TOT_PATHS,DENOT_TOT_REACH_CALLS,DENOT_MAX_ITERATIONS,DENOT_RESULT,"
...
if [[ $semantics == "inf" || $semantics == "fin" ]]; then
  stats_format="%Dd[0],%Di[0],%Dx[0],%Dp[0],%Drc[0],%Dmi[0],%Dr[0],%As[0],%Ais[0],%Aas[0],%Ae[0],%Atp[0],%Amp[0],%ANr[0],%Ascc[0],%Ax[0],%ATo[0],%ATx[0],%ATs[0],%ATis[0],%ATe[0],%ATscc[0]"
else
...
for ((tanks = 2 ; tanks <= max_tanks ; tanks++)); do
  ...
  for ((alternations = 1 ; alternations <= max_alternations ; alternations++)); do
    ...
    start=$(microtime)
    polyhedral_system=$(sys-gen gap -t "$tanks" -m "$time" -g "$gap")
    ...
    if [[ $tanks == 2 ]]; then
        poly_result=$(echo "$rtl_mc_result" | cut -d',' -f"7")
