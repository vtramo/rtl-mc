...
TEST_CASE("t0 & G(t1) & F(p & F(q)) with HIGH optimization")
{
    ...
    REQUIRE(predecessors(backwardNfa, stateTwo) == std::unordered_set<int>{ 6, 8 });
    ...
    REQUIRE(predecessors(backwardNfa, stateThree) == std::unordered_set<int>{ 6, 8 });
    ...
    REQUIRE(predecessors(backwardNfa, stateSix) == std::unordered_set<int>{ 0, 2, 4 });
    ...
    REQUIRE(predecessors(backwardNfa, stateEight) == std::unordered_set<int>{ 1, 5 });
