...
TEST_CASE("t0 & G(t1) & F(p & F(q)) with HIGH optimization")
{
    ...
    REQUIRE(backwardNfa.totalFinalStates() == 3);
    REQUIRE(backwardNfa.finalStates() == std::unordered_set<int>{ 11, 4, 2 });
    ...
    REQUIRE(predecessors(backwardNfa, stateThree) == std::unordered_set<int>{ 9, 7 });
    ...
    REQUIRE(predecessors(backwardNfa, stateFour) == std::unordered_set<int>{ 7, 9 });
    ...
    REQUIRE(predecessors(backwardNfa, stateFive) == std::unordered_set<int>{ 10 });
    ...
    REQUIRE(predecessors(backwardNfa, stateSix) == std::unordered_set<int>{ 10 });
    ...
    REQUIRE(predecessors(backwardNfa, stateSeven) == std::unordered_set<int>{ 0, 3, 5 });
    ...
    REQUIRE(nineStateDenotation.formula() == spot::parse_infix_psl("p & !q & t1").f);
    ...
    REQUIRE(predecessors(backwardNfa, stateNine) == std::unordered_set<int>{ 1, 6 });
    ...
    REQUIRE(tenStateDenotation.formula() == spot::parse_infix_psl("!p & t1").f);
    ...
    REQUIRE(predecessors(backwardNfa, stateEleven) == std::unordered_set<int>{ 8 });
}
