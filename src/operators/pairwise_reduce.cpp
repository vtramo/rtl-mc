...
typedef std::unordered_set<Poly*, Poly_hash_t, Poly_hash_t> poly_set_t;

typedef poly_set_t adjacents_t;

// A map from a polyhedra pointer to a set of polyhedra pointers (adjacency map)
typedef std::unordered_map<Poly*, adjacents_t, Poly_hash_t, Poly_hash_t> adj_map_t;
// A map from a polyhedra pointer to a polyhedra (closure map)
typedef std::unordered_map<Poly*, Poly, Poly_hash_t, Poly_hash_t> closure_map_t;
// A list of polyhedra pointers
typedef std::list<Poly*> poly_list_t;
...
void init_map(Powerset &ps, adj_map_t &adj) {
  ...
  std::cout << "(init:" << list.size() << ")" << std::flush;
  int count = 0; // DEBUG
...
static void merge_maps(adj_map_t &a, adj_map_t &b)
{
  ...
  std::cout << "(merging " << a_list.size() << "x" << b_list.size() << ")" << std::flush;
...
static void join_patches(Poly *p_point, Poly *q_point, adj_map_t &adj, poly_set_t &queue)
{
  ...
  for (adjacents_t::iterator it = adj[q_point].begin(); it != adj[q_point].end(); ++it) {
      Poly *r_point = *it;
      // std::cout << "Removing " << *q_point << " from adjacents of " << *r_point << std::endl;
...
void pairwise_reduce_map(adj_map_t &adj)
{
  ...
  while (!queue.empty()) {
    ...
    // std::cout << "[" << queue.size() << "/" << p_point->generators().total_memory_in_bytes() << "]" << std::flush;
    ...
    for (adjacents_t::iterator it = adj[p_point].begin(); it != adj[p_point].end(); ++it) {
      ...
      // std::cout << "checking " << *p_point << "(" << p_point << ") and " << *q_point << "(" << q_point << ") ..." << std::flush;

      if (p_point->upper_bound_assign_if_exact(*q_point)) {
	// it is, so replace both of them with their union
	// std::cout << "combining them!" << std::endl;
...
void pairwise_reduce_hyerarchical(Powerset &poly)
{
  ...
  std::cout << "(n0: " << n  << ")" << std::flush;

  // Continue until there is only one batch
  while (batches>1) {
    int i = 0;
    Powerset batch(poly.space_dimension(), PPL::EMPTY), result(poly.space_dimension(), PPL::EMPTY);
...
static void pairwise_reduce_with_bound_and_map(Powerset &poly, const int max_size, adj_map_t &map)
{
  ...
  if (n<=max_size) {
    ...
    Powerset  a(poly.space_dimension(), PPL::EMPTY), b(poly.space_dimension(), PPL::EMPTY);
    int i = 0, half = n/2;
...
{
  const PPL::dimension_type dim = dest.space_dimension();
  poly_list_t list_a, list_b;
  Powerset temp(dim, PPL::EMPTY), ab(dim, PPL::EMPTY);
...
void metaPairwiseReduce(Powerset &powerset, const bool check_correctness /* = false */)
{
  ...
  */
  ...
  if (!poly1.geometrically_covers(poly)) {
    std::cout << "RAW smaller than PPL!" << std::endl;
    exit(1);
  }
  if (!poly.geometrically_covers(poly1)) {
    std::cout << "PPL smaller than RAW!" << std::endl;
    exit(1);
  }
  */
  ...
  if (check_correctness) {
    if (!poly3.geometrically_covers(powerset)) {
      std::cout << "HPPL smaller than PPL!" << std::endl;
      exit(1);
    }
    if (!powerset.geometrically_covers(poly3)) {
      std::cout << "PPL smaller than HPPL!" << std::endl;
      exit(1);
    }
  }
}
