#include "DenotRecursiveMemoization.h"
#include "logger.h"
#include "reach.h"
#include "Timer.h"
#include "ppl_utils.h"

PowersetUniquePtr DenotRecursiveMemoization::run()
{
    m_iterations = 0;
...
PowersetUniquePtr DenotRecursiveMemoization::denot(
    int state,
    const Poly& P,
    const Poly& X,
...
)
{
    ...
    Log::log(Verbosity::trace, "State: {}\n{}", state, stateDenotation.toString(m_polyhedralSystem->symbolTable()));
    ...
    Log::log(Verbosity::trace, "Visited region V[{}]: {}",
        state,
        PPLOutput::toString(getVisitedPowerset(V, state), m_polyhedralSystem->symbolTable())
    );
    ...
    for (const auto& edgePredecessor: m_backwardNfa.predecessors(state))
    {
        ...
        Log::log(Verbosity::trace, "Predecessor denotation minus visited region (A): {}",
            PPLOutput::toString(*A, m_polyhedralSystem->symbolTable()));
        ...
        for (const auto& [Q, Y]: reachPairs)
        {
            ...
            PowersetUniquePtr denotResult {
                m_denotResults[predecessor].count(Y.hash_code()) == 1
                    ? std::make_unique<Powerset>(m_denotResults[predecessor].at(Y.hash_code()))
                    : denot(predecessor, Q, Y, V, recursionDepth + 1, !isSing)
            };
    ...
    m_denotResults[state].emplace(X.hash_code(), *result);
    return result;
}

void DenotRecursiveMemoization::addDisjunct(std::vector<Powerset>& V, const int state, const Poly& P)
{
    V[state].add_disjunct(P);
}

const Powerset& DenotRecursiveMemoization::getVisitedPowerset(std::vector<Powerset>& V, const int state)
{
    return V[state];
}