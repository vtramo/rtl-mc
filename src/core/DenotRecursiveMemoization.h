...
class DenotRecursiveMemoization: public Denot
{
public:
      ...
      , m_backwardNfa { backwardNfa }
    {
        ...
        m_denotResults.resize(m_backwardNfa.totalStates());
    }
    ~DenotRecursiveMemoization() override = default;
...
private:
    using PowersetHashCode = int32_t;
    using DenotResultBySubsetPatchMap = std::unordered_map<PowersetHashCode, Powerset>;

    int m_iterations { };
    PolyhedralSystemConstSharedPtr m_polyhedralSystem {};
    const BackwardNFA& m_backwardNfa {};
    int m_maxRecursionDepth {};
    std::vector<DenotResultBySubsetPatchMap> m_denotResults {};
